...
// aggregation across users/readingSessions/sessions - at 1 000 concurrent
// loads that's a query storm on a table that barely changes second-to-second.
const LEADERBOARD_TTL = 120;
...
export async function GET(req: NextRequest) {
  try {
    ...
    const cacheKey = `leaderboard:${filter}:${filter === "department" ? user.departmentId : "global"}`;

    const leaderboard = await withCache(cacheKey, LEADERBOARD_TTL, async () => {
