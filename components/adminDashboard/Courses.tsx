...
const Courses: React.FC<CoursesProps> = ({
  ...
  return (
    <div>
      ...
      {/* Courses Table */}
      <div
        className={`gap-4 bg-white dark:bg-zinc-950 px-8 py-5 rounded-md ${open && "no-scrollbar"}`}
      >
        <div className="flex justify-between items-center mb-10">
          <h3 className="font-open-sans font-semibold ">List of Courses</h3>
          <button
            className="flex gap-2 items-center font-medium font-poppins text-white cursor-pointer rounded-full px-3 py-2 text-xs border border-zinc-700 bg-zinc-600"
        ...
        <div className="w-full overflow-x-auto">
          <table className="table-auto min-w-[500px] md:min-w-full border-collapse">
            <thead className="text-left">
              <tr className="tracking-wider uppercase text-zinc-400 text-xs font-karla border-b border-zinc-200">
            ...
            <tbody>
              {courses?.map((course) => (
                <tr
                  key={course.id}
                  className="font-poppins text-xs py-3 text-zinc-800 dark:text-zinc-200 font-normal border-b border-zinc-200 dark:border-zinc-800"
                >
                  ...
                  <td className="flex pt-2 gap-1.5">
                    ...
                    <div className="p-2 cursor-pointer bg-zinc-500 text-white rounded">
